...
from rename_project import safe_console

PKG = Path(__file__).resolve().parent.parent / "src" / "rename_project"
...
class TestEchoOnALegacyCodepage:
    ...
    def test_a_character_cp1252_lacks_degrades_to_its_ascii_form(
        self, glyph: str, expected: str
    ) -> None:
        stream = _cp1252_stream()
...
class TestNoModuleBypassesTheAdapter:
    ...
    def test_no_module_builds_an_unwrapped_rich_console_on_stdout(self) -> None:
        ...
        assert not offenders, (
            f"wrap the writer with safe_console.safe_stream(sys.stdout): {offenders}"
        )
